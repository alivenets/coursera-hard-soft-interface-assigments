...
static void updateBoundaryTag(BlockInfo *blockInfo)
{
...
void* mm_malloc (size_t size) {
  ...
  precedingBlockUseTag = ptrFreeBlock->sizeAndTags & (size_t)TAG_PRECEDING_USED;

  if (SIZE(ptrFreeBlock->sizeAndTags) - reqSize >= MIN_BLOCK_SIZE) {
    // Split block if it is greater than required size
    nextFreeBlock = (BlockInfo*)(POINTER_ADD(ptrFreeBlock, reqSize));
    nextFreeBlock->sizeAndTags = SIZE(ptrFreeBlock->sizeAndTags) - reqSize;
    nextFreeBlock->sizeAndTags |= (size_t)TAG_PRECEDING_USED;
    updateBoundaryTag(nextFreeBlock);

    insertFreeBlock(nextFreeBlock);

    ptrFreeBlock->sizeAndTags = reqSize;
    updateBoundaryTag(ptrFreeBlock);
  }
  else {
    // Mark next block with TAG_PRECEDING_USED
    BlockInfo *nextBlock = (BlockInfo*)POINTER_ADD(ptrFreeBlock, SIZE(ptrFreeBlock->sizeAndTags));

    // Mark that the prevous block is unused (required for coalescing)
    nextBlock->sizeAndTags |= (size_t)TAG_PRECEDING_USED;

    // Update boundary tag if block is unused (not to corrupt data in the block)
    if (!(nextBlock->sizeAndTags & TAG_USED)) {
      updateBoundaryTag(nextBlock);
    }
  }

  removeFreeBlock(ptrFreeBlock);

  ptrFreeBlock->sizeAndTags |= (size_t)(TAG_USED | precedingBlockUseTag);
...
void mm_free (void *ptr) {
  ...
  BlockInfo * preceedingBlock = NULL;
  size_t precedingBlockUseTag;

  // Implement mm_free.  You can change or remove the declaraions
  // above.  They are included as minor hints.

  // Free block, mark it as free.
  blockInfo = POINTER_SUB(ptr, WORD_SIZE);

  blockInfo->sizeAndTags &= ~(size_t)TAG_USED;
  updateBoundaryTag(blockInfo);

  followingBlock = POINTER_ADD(blockInfo, SIZE(blockInfo->sizeAndTags));

  // Mark that the previous block is unused (required for coalescing)
  followingBlock->sizeAndTags &= ~(size_t)TAG_PRECEDING_USED;

  // Update boundary tag if block is unused (not to corrupt data in the block)
  if (!(followingBlock->sizeAndTags & TAG_USED))
     updateBoundaryTag(followingBlock);
